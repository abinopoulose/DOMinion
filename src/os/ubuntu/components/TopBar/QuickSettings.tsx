  ...
  return (
    <div className="quick-settings" onClick={(e) => { e.stopPropagation(); setActivePillMenu(null); setShowPowerMenu(false); }}>
          ...
          <div style={{ position: 'relative' }}>
            {showPowerMenu && (
              <div className="qs-power-menu">
                ...
                {!isLoginScreen && <div className="qs-power-menu-item" onClick={() => { openDialog('log_out'); onClose(); }}>Log Out…</div>}
              </div>
            )}
      ...
      <div className="quick-settings__pills">
        {pills.filter(pill => !isLoginScreen || ['wifi', 'bt', 'airplane'].includes(pill.id)).map((pill, index) => (
